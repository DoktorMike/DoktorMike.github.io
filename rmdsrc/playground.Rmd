...
```{stan output.var="flugga3", eval=FALSE}
data {
  int< lower = 0 > N;       // Number of data points
  vector[N] y;          // The response variable
  matrix[N, 7] xweekday; // The weekdays variables
...
sfitarr<-as.array(sfit) 
tmpnames<-c("b0", grep("bweekdayhat\\[", sfitarr[1,2,] %>% names, value=T))
# knitr::kable(tmpnames)
d<-sapply(rstan::extract(sfit, pars=tmpnames), function(x){ c(Estimate=mean(x), SD=sd(x)) })
...
So from a model estimation standpoint we should be pretty happy now. 

But how does this new estimation compare to the others? Below I will repeat the model performance table from earlier and extend it with our new "Bayes2" estimation. 
...
It's evident that our new way of estimating the parameters of the model yields not only a more satisfying modeling approach but also provides us with a superior model from a performance perspective. I'd call that a win win. 
