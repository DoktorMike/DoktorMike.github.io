...
postdf %>% mutate(s=1:10) %>% gather(x, y, -s) %>% mutate(x=as.numeric(gsub("X", "", x)), s=as.factor(s)) %>% ggplot(aes(y=y, x=x, group=s)) + geom_point(color="magenta", alpha=0.5) + theme_minimal()
```
